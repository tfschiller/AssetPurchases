...
In general this type of commitment signals that monetary policy will be kept loose for a certain duration, with agents having an understanding of the conditions under which monetary policy will be tightened. This should further reduce long-term interest rates as agents know short term rates will be lower for longer.  Specifically, if the Federal Reserve commits to buying assets at a future date, then the price of these assets will rise now (otherwise arbitrage is possible: agents could buy these assets now and sell them to the Federal Reserve at a higher price at the future date), suppressing yields on these assets. Also, a commitment to continue a LSAP programme indicates interest rates will be held lower for longer (\hyperlink{gangnon2011}{Gagnon et al. 2011}). 

The intention of LSAP programmes is to depress interest rates and increase asset prices across different asset classes and maturities in order to achieve the Federal Reserve's macroeconomic objectives. Lower interest rates and higher asset prices will help to increase consumption and investment in the economy. LSAP programmes, through the aforementioned portfolio rebalancing and signalling effects, lead to increases in the wealth of households and the value of firms. Households therefore increase their consumption and firms their investment, spurring economic growth and employment.
...
\hyperlink{rosa2012}{Rosa (2012)} uses an 'event study approach' and calculates the monetary policy shock by analysing copies of the Financial Times before and after FOMC meetings to determine the extent to which LSAP programmes were anticipated by agents. \hyperlink{rosa2012}{Rosa (2012)} finds that the effect on asset prices (Treasuries of different maturities, Five Year and Ten Year Treasury Inflation-Protected Securities and S\&P's 500 cash index) is not statistically different from an unanticipated cut in the FOMC target rate. The effect on prices is not explicitly reported. 
...
However, \hyperlink{hesse2018}{Hesse et al.'s (2018)} finding that the impact of later LSAP programmes may be reduced because of anticipation by agents reduces the justification for using an event study approach. Some of the effect of the LSAP programme is missed by only considering a short time frame around announcement. Therefore an approach which does not restrict the time frame is justified (see \hyperlink{balatti2016}{Balatti, Brooks, Clements and Kappou (2016)} for a wider discussion of the drawbacks of using an arbitrary event window).
...
As in \hyperlink{domanski2016}{Domanski et al. (2016)}, possible shifts in portfolio composition by household are abstracted from (due to a lack of regular data on the composition of household wealth). This means that this is a partial equilibrium exercise, where households are assumed not to shift their portfolios based on macroeconomic factors. It is worth noting however, that comparisons of household surveys across time suggest portfolio composition remains fairly stable (\hyperlink{domanski2016}{Domanski et al. (2016)}). However, this conflicts with the supposed portfolio rebalancing effects that result from LSAP programmes. In addition, the quantitative impact of LSAP programmes on employment and incomes is left out of scope. As noted, \hyperlink{bunn2018}{Bunn, Pugh and Yeates (2018)} find this to be an important factor in reducing inequality. 



# Conclusions

After the GFC, the FOMC cut interest rates to zero and implemented a number of LSAP programmes. There has been much debate about the financial market and macroeconomic effects of this unconventional monetary policy. Here, the effects on asset prices and inequality have been analysed using a VAR model. The responses of certain assets - the Ten Year Treasury yield, the median US house price and the Wilshire 5000 index - to an expansion in the Federal Reserve's balance sheet have been computed. Then, the balance sheets for households along the income distribution are constructed. This means some insight into how LSAP programmes affect the (net) wealth of different household can be gained. 

The ultimate objective of the Federal Reserve's LSAP programmes is to improve the macroeconomy. Real GDP and inflation expectations show a sustained rise in response to a balance sheet expansion. These are important consideration when assessing the impact on inequality, specifically whether LSAP programmes have improved the economy in such a way as to increase employment and incomes among those in poorer households. There is scope to extend this analysis in that direction. 

There is a sustained increase in the median US house price as a result of an expansionary Federal Reserve balance sheet shock. This will tend to increase wealth inequality as the richest households have greater absolute levels of real estate wealth. However, poorer households have greater real estate leverage so increases in house prices, with decreases in the cost of debt, tends to decrease wealth inequality.\
\
\
...
ggplot(MedianSalesPriceHouses)+ 
  ...
  theme(axis.title=element_text(size=8, family="LM Roman 10"), axis.text=element_text(size=7, family="LM Roman 10"))+
  ggtitle("Figure 5: Median Sales Price of Houses Sold for the United States")+theme(plot.title = element_text(size=8, family="LM Roman 10"))+
  theme(
    panel.grid.major.x = element_blank() ,
    panel.grid.major.y = element_line( size=.3, color="grey" ))+
  theme(axis.ticks.y = element_blank())+
...
ggplot(TenYearTreasuryConstantMaturity)+ 
  ...
  scale_y_continuous(labels=comma, name="Percent", breaks = scales::pretty_breaks(n = 10))+
  theme(axis.title=element_text(size=8, family="LM Roman 10"), axis.text=element_text(size=7, family="LM Roman 10"))+
  ggtitle("Figure 6: 10 Year Treasury Constant Maturity Rate")+theme(plot.title = element_text(size=8, family="LM Roman 10"))+
